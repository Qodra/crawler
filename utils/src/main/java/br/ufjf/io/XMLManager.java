...
public class XMLManager {
    ...
    public static final String getValue(String[] setOfTags)
    {
        String result= "";

        if (nList == null) return result;
        
        NodeList children = nList;
        
        for (String tag : setOfTags) {
            children = ((Element) children.item(0)).getElementsByTagName(tag);
        }
        
        result = ((Element) children.item(0)).getTextContent().trim();
